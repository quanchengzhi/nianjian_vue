<template>
  <div class="details_wrapper clearfix">
    ...
    <div class="delivery_info">
      <h4>接车信息</h4>
      <p v-if="firstNum.indexOf(state) == -1">
        <span>接车师傅</span>
        <i>陈师傅</i>
      </p>
      <p v-if="firstNum.indexOf(state) == -1">
    ...
    >
      ...
      <router-link v-if="state == '0'" to="/pay" class="right_btn"
        >去支付</router-link
      >
      <a v-if="state != '0'" :href="callHref" class="right_btn">{{callText}}</a>
    </div>
    <div v-if="state == '1' || state == '3'" class="big_btn">申请退款</div>
    <a v-if="state == '6'" href="tel:00000000" class="big_btn1">联系师傅</a>
  </div>
</template>
  ...
  methods: {
    ...
    changeState() {
      ...
      if (this.state == "0") {
        ...
        this.callText = "车辆已归还";
        this.callHref = "javascript:void(0);";
