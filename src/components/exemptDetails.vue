<template>
  <div class="details_wrapper clearfix">
    ...
    <h3 v-if="state == '0' || state == '6'" class="details_waitpay">
      {{ detailsWaitpay }}
    </h3>
    <div class="detail_head">
      ...
      <p class="receive_address">
        <span>收件地址</span>
        <i>广东省深圳市宝安区西乡盐田宝安大道4004号旭生大厦21楼</i>
      </p>
      <p class="inspection_info">
        <span>年检资料</span>
        <span>
    ...
    <div v-if="state == '5'" class="delivery_info">
      <h4>快递信息</h4>
      <p class="inspection_mark">
        <span>年检标</span>
        <img src="../assets/img/bg4.jpg" alt="" />
      </p>
      <p class="delivery_company">
        <span>快递公司</span>
        <i>圆通快递</i>
      </p>
      <p class="delivery_num">
        <span>快递单号</span>
    ...
    <div v-if="state == '0' || state == '2'" class="detail_btn">
      <router-link to="/index" class="left_btn">{{ leftBtn }}</router-link>
      <router-link :to="rightRouter" class="right_btn">{{
        rightBtn
      }}</router-link>
    </div>
    <div v-if="state == '1' || state == '3'" class="big_btn">申请退款</div>
  </div>
</template>
  ...
  methods: {
    ...
    changeState() {
      ...
      if(this.state=='0'){
        ...
        this.rightRouter = "/supplementInfo";
